...
if (isset($_POST['add_achievement'])) {
    ...
    if (empty($achievement)) {
        $error = "⚠ Please enter an achievement.";
    } else {
        $stmt = $conn->prepare("INSERT INTO achievements (achievement) VALUES (?)");
        $stmt->bind_param("s", $achievement);
        if ($stmt->execute()) {
...
if (isset($_GET['delete'])) {
    ...
    header("Location: about_achivements.php");
    exit();
}

// ✅ Fetch Achievements
$achievements = [];
$res = $conn->query("SELECT * FROM achievements ORDER BY id DESC");
...
<div class="main-content">
    ...
    <form method="POST">
        <textarea name="achievement" placeholder="Enter achievement here..." style="flex:1 1 100%;padding:10px;border-radius:6px;border:1px solid #ccc;" required></textarea>
        <button type="submit" name="add_achievement">Add Achievement</button>
    </form>

    <h3 style="color:#06389d;margin:20px 0;">Achievements List</h3>
    <?php if (empty($achievements)): ?>
        <p>No achievements added yet.</p>
    <?php else: ?>
        <table>
            <thead>
                <tr><th>ID</th><th>Achievement</th><th>Delete</th></tr>
            </thead>
            <tbody>
            <?php foreach ($achievements as $ach): ?>
                <tr>
                    <td data-label="ID"><?= $ach['id'] ?></td>
                    <td data-label="Achievement"><?= htmlspecialchars($ach['achievement']) ?></td>
                    <td><a href="?delete=<?= $ach['id'] ?>" class="delete-btn" onclick="return confirm('Delete this achievement?')">Delete</a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
